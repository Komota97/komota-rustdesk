...
function(x_vcpkg_pkgconfig_get_modules)
    ...
    # Komota overlay: VCPKG_HOST_EXECUTABLE_SUFFIX reflects the *actual* running
    # host OS (Linux -> ""), not our --host-triplet=x64-mingw-static override, so
    # it omits ".exe" even though the pkgconf host tool was actually cross-built
    # as a real Windows PE binary for that triplet. Fall back to the .exe path
    # when the suffix-less one doesn't exist and we're cross-compiling to Windows.
    if(NOT EXISTS "${PKGCONFIG}" AND NOT CMAKE_HOST_WIN32 AND VCPKG_TARGET_IS_WINDOWS AND EXISTS "${PKGCONFIG}.exe")
        set(PKGCONFIG "${PKGCONFIG}.exe")
    endif()
...
function(z_vcpkg_set_pkgconfig_path _path _backup)
    # Komota overlay: when cross-compiling to a Windows/mingw triplet from a
    # non-Windows host, PKGCONFIG above resolves to a cross-built pkgconf.exe
    # that runs via Wine/binfmt. That binary is a plain mingw-w64 program (not
    # Cygwin-aware), so it can't resolve Unix-style paths at all -- it needs
    # Windows-style paths. Wine maps the Unix root to the Z: drive by default,
    # so translate "/a/b" -> "Z:\a\b" (and ":" list-separators -> ";") in that case.
    set(_join_sep "${VCPKG_HOST_PATH_SEPARATOR}")
    if(NOT CMAKE_HOST_WIN32 AND VCPKG_TARGET_IS_WINDOWS)
        # VCPKG_HOST_PATH_SEPARATOR reflects the *actual* running host (Linux -> ":"),
        # not our --host-triplet=x64-mingw-static override, but the paths below are
        # being translated for consumption by a cross-built Windows pkgconf.exe, which
        # needs ";" regardless of what the real host's separator convention is.
        set(_join_sep ";")
        string(REPLACE ":" ";" _path "${_path}")
        string(REPLACE "/" "\\" _path "${_path}")
        string(REGEX REPLACE "(^|;)\\\\" "\\1Z:\\\\" _path "${_path}")
        if(NOT _backup STREQUAL "")
            string(REPLACE ":" ";" _backup "${_backup}")
            string(REPLACE "/" "\\" _backup "${_backup}")
            string(REGEX REPLACE "(^|;)\\\\" "\\1Z:\\\\" _backup "${_backup}")
        endif()
    endif()
    if(NOT _backup STREQUAL "")
        set(ENV{PKG_CONFIG_PATH} "${_path}${_join_sep}${_backup}")
    else()
        set(ENV{PKG_CONFIG_PATH} "${_path}")
        message(STATUS "KOMOTA-DEBUG z_vcpkg_set_pkgconfig_path (no backup) => [$ENV{PKG_CONFIG_PATH}]")
    endif()
endfunction()
